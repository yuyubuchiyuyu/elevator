...
public class Calculation {
    ...
    public static ArrayList<Result> find_derivative(ArrayList<Result> exp) {
        ArrayList<Result> results = new ArrayList<>();
        for (int i = 0; i < exp.size(); i++) {
            Result old = exp.get(i); // a*x^b*exp(...)
            BigInteger oldIndex = exp.get(i).getIndex(); // b
            BigInteger oldCoefficient = exp.get(i).getCoefficient(); // a
            ArrayList<Result> oldExp = exp.get(i).getExp(); // exp(...)中的...
            // a*x^b*exp(...) -> a*b*x^(b-1)*exp(...) + a*x^b*exp(...)*dx(...)
            // a*b*x^(b-1)*exp(...)
            Result result1 = new Result(oldCoefficient.multiply(oldIndex),
                    oldIndex.subtract(BigInteger.valueOf(1)));
            result1.addExp(oldExp);
            int sign = searchIndex(result1.getIndex(), result1.getExp(), results);
            if (sign == -1) {
                results.add(result1);
            } else {
                results.get(sign).changeCoefficient(
                        results.get(sign).getCoefficient().add(result1.getCoefficient()));
            }
            // a*x^b*exp(...)*dx(...)
            ArrayList<Result> base = new ArrayList<>();
            base.add(old);
            String newStr = '(' + Operation.giveResult(base) + ')' + "*"
                    + '(' + Operation.giveResult(find_derivative(oldExp)) + ')';
            Expr newExpr = new Parser(new Lexer(newStr)).parserExpr();
            ArrayList<Result> result2 = Operation.deleteResult(newExpr.getResults());
            for (int j = 0; j < result2.size(); j++) {
                sign = searchIndex(result2.get(j).getIndex(), result2.get(j).getExp(), results);
                if (sign == -1) {
                    results.add(result2.get(j));
                } else {
                    results.get(sign).changeCoefficient(
                            results.get(sign).getCoefficient().add(
                                    result2.get(j).getCoefficient()));
                }
            }
        }
        return results;
